...
public class TinyGoogle {
	...
	public static void main(String args[]) throws IOException, IllegalArgumentException, ClassNotFoundException, InterruptedException {
		...
		Path temp_out = new Path("/InvertedIndexOutput_Spark");
		Path temp_out2 = new Path("/InvertedIndexOutput_Spark_query");
		...
		System.out.println("Welcome to TinyGoogle, Spark edition!\n");
	...
	private static void createBooksAndIndex() throws IOException, InterruptedException {
	...
	private static boolean index() throws IOException, InterruptedException {
		
		SparkAppHandle spark = new SparkLauncher()
				.setAppResource("/Users/anthonytummillo/Desktop/CS1699/Project2/spark-scala-maven-boilerplate-project/target/spark-scala-maven-project-0.0.1-SNAPSHOT-jar-with-dependencies.jar")
				.setMainClass("com.examples.InvertedIndex")
				.setMaster("yarn")
				.addAppArgs(books.toString(), output.toString())
		...
		if(spark.getState() == SparkAppHandle.State.FINISHED){
			return true;
		}else{
			return false;
		}
	...
	public static void executeQuery(String query, HashMap<String, Long> sizes) throws IllegalArgumentException, IOException, ClassNotFoundException, InterruptedException{
		...
		SparkAppHandle spark = new SparkLauncher()
				.setAppResource("/Users/anthonytummillo/Desktop/CS1699/Project2/spark-scala-maven-boilerplate-project/target/spark-scala-maven-project-0.0.1-SNAPSHOT-jar-with-dependencies.jar")
				.setMainClass("com.examples.ExecuteQuery")
				.setMaster("yarn")
				.addAppArgs(output.toString() + "/index", queryOutput.toString(), query)
				.setAppName("ExecuteQuery")
				.startApplication();
		...
		while(line != null){
			...
			nameArray = new String[(lineArray.length-1)/2];
			countArray = new String[(lineArray.length-1)/2];
			term = lineArray[0];
			...
			for(int i = 1; i<lineArray.length; i++){
				if(i%2 == 1){
					nameArray[nameIndex] = lineArray[i];
					nameIndex++;
				}else{
					countArray[countIndex] = lineArray[i];
					countIndex++;
				}
			...
			for(int i = 0; i<nameArray.length; i++){
				termMap.put(nameArray[i], calcTFIDF(Long.parseLong(countArray[i]), sizes.get(nameArray[i]), numTotalDocs, nameArray.length-1));
			}
			
			indexMap.put(term, termMap);
		...
		for(Map.Entry<String, HashMap<String,Double>> entry : indexMap.entrySet()){
			...
			for(Map.Entry<String, Double> docMap : termScores.entrySet()){
				...
				tuple dwlEntry = new tuple(term2, tmpWeight);
